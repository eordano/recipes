...
### Trap 13 -- `node_modules` and `dist` in `src` perturb the FOD hash and bloat the store

`fetchPnpmDeps` hashes `src`, so anything sitting in your working tree at build
time is part of the input to the FOD. A local `pnpm install` / `pnpm build` you
ran while developing leaves a `node_modules` symlink forest and a `dist/` under
`src`, and both then travel into the fetcher: the hash moves the moment they
change (a re-install rewrites `.pnpm`, a rebuild rewrites `dist`), and the store
path swells with a whole second copy of the dependency tree you are also
fetching offline. Proven building a real Vite SPA -- the deps hash was unstable
run-to-run until the working-tree `node_modules` was filtered out.

Filter them before hashing with `lib.cleanSourceWith`:
...
src = lib.cleanSourceWith {
  ...
  filter = path: type:
    ...
    in base != "node_modules" && base != "dist";
};
```

Pass that same filtered `src` to `fetchPnpmDeps` (via `inherit (finalAttrs)
src`, as the examples do) so the fetcher and the build hash the identical tree.
...
- **Bare `pkgs.pnpm` works only until the alias advances.** Using `pnpm`
  instead of pinning the major (e.g. `pnpm_11`) is fine while the `pnpm` alias
  *equals* the major whose lockfile format you have -- but when the alias moves
  to the next major, the store format changes under you and your `pnpmDeps` hash
  moves with it. Pin the major.
