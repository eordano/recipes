...
in
{
  options.services.push-observability-receiver = {
    enable = mkEnableOption "push-based logs + metrics + Grafana receiver";
    ...
    dataDir = mkOption {
      ...
      description = "Directory holding the log/metric databases and the Grafana DB. Put it on persistent storage.";
    };
    ...
    backend = mkOption {
      ...
      description = ''
        Which pair of databases Vector fans into. `loki-prometheus` runs Loki
        and Prometheus; `victoria` runs VictoriaLogs and VictoriaMetrics in
        their place and relocates both onto `dataDir` (the upstream modules
        would otherwise put them in a DynamicUser StateDirectory).

        The Grafana datasource uids are `loki` and `prometheus` under both, so
        dashboards and alert rules need no uid changes, and PromQL is unchanged
        because VictoriaMetrics serves it at /prometheus. LogQL is NOT: under
        `victoria` the log datasource is the VictoriaLogs Grafana plugin and
        every log query must be written in LogsQL.
      '';
    };
    ...
    lokiPort = mkOption {
      type = types.int;
      default = 3100;
      description = "Loki HTTP API port (loopback only; backend = loki-prometheus).";
    };
    ...
    prometheusPort = mkOption {
      type = types.int;
      default = 9090;
      description = "Prometheus HTTP + remote-write port (loopback only; backend = loki-prometheus).";
    };

    victoriaLogsPort = mkOption {
      type = types.int;
      default = 9428;
      description = "VictoriaLogs HTTP port (loopback only; backend = victoria).";
    };

    victoriaMetricsPort = mkOption {
      type = types.int;
      default = 8428;
      description = "VictoriaMetrics HTTP + remote-write port (loopback only; backend = victoria).";
    };

    nodeExporterPort = mkOption {
      type = types.int;
      default = 9100;
      description = "Port for the local node exporter the metrics database scrapes.";
    };

    retentionPeriod = mkOption {
      type = types.str;
      default = "168h";
      description = "Log retention period (Loki `retention_period`, or VictoriaLogs -retentionPeriod).";
    };

    metricsRetentionPeriod = mkOption {
      type = types.str;
      default = "30d";
      description = "Metrics retention period (Prometheus `retentionTime`, or VictoriaMetrics -retentionPeriod).";
    };
    ...
    logLevel = mkOption {
      ...
      default = "info";
      description = ''
        Log level for the databases and Grafana. The Victoria binaries have no
        DEBUG level, so `debug` is passed to them as INFO.
      '';
    };
    ...
    extraScrapeJobs = mkOption {
      ...
      default = [ ];
      description = ''
        Additional Prometheus `scrape_configs` entries, merged into whichever
        metrics database is active (Prometheus, or VictoriaMetrics' promscrape).
        Use for sources that can't push via Vector remote-write -- exporters on
        hosts you don't control, or anything behind a reverse proxy. Attach a
        `host` label in `static_configs[].labels` so dashboards'
        `{host=~"$host"}` filter picks it up.
      '';
      example = literalExpression ''
        [{
          job_name = "ups";
  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    })
    {
      ...
      ++ optionals useVictoria [
        "d ${cfg.dataDir}/victorialogs 0700 ${cfg.user} ${cfg.user} - -"
        "d ${cfg.dataDir}/victoriametrics 0700 ${cfg.user} ${cfg.user} - -"
      ]
      ++ [
        ...
        settings = mkMerge [
          ...
          (mkIf useVictoria {
            sinks.victorialogs = {
              type = "loki";
              inputs = [ "process_logs" ];
              endpoint = "http://127.0.0.1:${toString cfg.victoriaLogsPort}";
            ...
            sinks.victoriametrics = {
              type = "prometheus_remote_write";
              inputs = [ "agent_metrics" ];
              endpoint = "http://127.0.0.1:${toString cfg.victoriaMetricsPort}/api/v1/write";
              healthcheck.enabled = false;
              request = {
                retry_initial_backoff_secs = 2;
                retry_max_duration_secs = 600;
      ...
      services.grafana = {
        ...
        provision = {
          datasources.settings.datasources = [
            {
              name = "Loki";
              type = if useVictoria then "victoriametrics-logs-datasource" else "loki";
              uid = "loki";
              url =
                if useVictoria then
                  "http://localhost:${toString cfg.victoriaLogsPort}"
                else
                  "http://localhost:${toString cfg.lokiPort}";
              access = "proxy";
              isDefault = true;
            }
            {
              ...
              url =
                if useVictoria then
                  "http://localhost:${toString cfg.victoriaMetricsPort}/prometheus"
                else
                  "http://localhost:${toString cfg.prometheusPort}";
              access = "proxy";
            }
          ];
    ...
    (mkIf useVictoria {
      ...
      services.victoriametrics = {
        enable = true;
        listenAddress = "127.0.0.1:${toString cfg.victoriaMetricsPort}";
          ...
          scrape_configs = [
            {
              ...
              static_configs = [
                {
                  targets = [ "localhost:${toString cfg.victoriaMetricsPort}" ];
                  labels.host = config.networking.hostName;
                }
              ];
            }
            {
              ...
              static_configs = [
                {
                  targets = [ "localhost:${toString cfg.victoriaLogsPort}" ];
                  labels.host = config.networking.hostName;
                }
              ];
            }
