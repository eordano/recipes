    ...
    assertions =
      ...
      ++ lib.mapAttrsToList (poolName: pool: {
        assertion =
          pool.mounts == { }
          || (
            lib.elem "noauto" pool.mountOptions
            && lib.any (o: lib.hasPrefix "x-systemd.wanted-by=" o) pool.mountOptions
          );
        message = ''
          services.zfsNativeKeys.pools.${poolName}.mountOptions must contain both
          "noauto" and an "x-systemd.wanted-by=" entry whenever `mounts` is set.
          Got: ${lib.generators.toPretty { } pool.mountOptions}

          Without them the generated mount stays ordered Before=local-fs.target
          while requiring ${pool.unitName}.service, which (carrying
          DefaultDependencies=yes) is ordered After=basic.target -> sysinit.target
          -> local-fs.target. That is an ordering cycle. systemd does not refuse
          to boot on it: it breaks the loop by DELETING jobs, logs "Found ordering
          cycle", and carries on looking healthy while mounts and anything
          ordered after local-fs.target are silently skipped.

          "x-systemd.automount" is NOT a substitute: it leaves the Before=
          edge in place. Use the module default
          [ "zfsutil" "noauto" "x-systemd.wanted-by=multi-user.target" ].

          Do NOT set DefaultDependencies=no on the key-load unit to break the
          cycle instead -- see the README table.
        '';
      }) cfg.pools;
