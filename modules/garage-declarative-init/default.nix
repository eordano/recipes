...
in
{
  options.services.garage-manager = {
    ...
    bootstrapPeers = mkOption {
      ...
      description = ''
        Other cluster nodes to bootstrap RPC discovery from, in Garage's
        `<node_id>@<host>:<port>` form. Only used in multi-node mode.

        The `<node_id>@` prefix is mandatory: Garage rejects a bare
        `host:port` entry with `Unable to parse and/or resolve peer hostname`
        and bootstraps from nothing. A node that has already met its peers
        keeps running from the persisted `<metadata_dir>/peer_list`, so a
        cluster can look healthy for months while bootstrap is dead -- the
        breakage only surfaces when a node joins for the first time or loses
        its metadata.

        A node's ID is the public half of the RPC keypair generated in
        `<metadata_dir>` on first start; read it with `garage node id -q`.
        It is stable for the life of that metadata directory and is not a
        secret.
      '';
    };
    ...
    capacity = mkOption {
      ...
      example = "100G";
      description = ''
        Storage capacity advertised to the Garage layout for this node. Only
        read when the node claims a layout role, i.e. when `assignLayoutRole`
        is true.

        Leaving this at the default on a real storage node is a quiet way to
        cripple a cluster: capacity is what Garage's replica placement divides
        up, so one node joining at 1G caps the usable size of its whole zone
        at 1G no matter how large its disk is.
      '';
    };

    assignLayoutRole = mkOption {
      type = types.bool;
      default = true;
      description = ''
        Whether this node may add itself to the cluster layout at `capacity`.

        Set false for a node that should join the cluster and gossip, but not
        store replicas -- a client-side or gateway member, or a machine whose
        disk budget is not settled yet. With this false the reconciler still
        commits staged changes made deliberately by an operator; it just never
        stages one for this node on its own.

        The default is true because that is what a single-node deployment
        needs. On a multi-node cluster, prefer deciding a node's capacity
        before it first starts, since a role claimed at the default is
        committed to the shared layout and has to be removed by hand.
      '';
    };
            ...
            if echo "$LAYOUT_STATUS" | grep -q "No nodes currently have a role"; then
              ...
              ${
                if cfg.assignLayoutRole then
                  ...
                  ''
                    echo "Cluster joined; not claiming a layout role (assignLayoutRole = false)."
                  ''
              }
            else
