...
let
  ...
  binaryRegex =
    ...
    if b.regex != null then
      ...
      let
        ...
        leaf = if b.wrapped then "(\\.)?${escapeRe parts.leaf}(-wrapped)?" else escapeRe parts.leaf;
  ...
  unrooted = builtins.filter (
    p: !(lib.hasPrefix "^/nix/store/" p.re || lib.hasPrefix "^/" p.re)
  ) processRegexes;
