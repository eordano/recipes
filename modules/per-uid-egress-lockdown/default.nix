...
let
  ...
  ++ map (
    p: ''meta skuid ${toString cfg.uid} oifname "lo" tcp dport ${toString p} accept''
  ) cfg.extraLoopbackPorts
  ++ [
  ...
  ++ map (
    p: "meta skuid ${toString cfg.proxyUid} tcp dport ${toString p} accept"
  ) cfg.proxy.egressPorts
  ++ [
  ...
  roMountArgs = mapAttrsToList (
    dest: host: bindArg "--ro-bind" (escapeShellArg host) dest
  ) cfg.launcher.roMounts;
  ...
  defaultLauncher = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      ${optionalString cfg.mountStage.enable ''[ -d "$STAGE" ] || { echo "mount stage $STAGE is not present (${cfg.mountStage.unitName})" >&2; exit 1; }''}
      ${concatStringsSep "\n" secretChecks}
