...
pkgs.testers.nixosTest {
  ...
  testScript = ''
    ...
    def test_cache_directory():
        ...
        cache.succeed("test -d /var/cache/pypi")

    def test_pip_configuration():
        """Test pip is configured to use cache"""
        client.succeed("test -f /etc/pip.conf")
        pip_config = client.succeed("cat /etc/pip.conf")
        assert "pypi-cache.test" in pip_config, "Cache URL not in pip config"
        assert "trusted-host" in pip_config, "Trusted host not configured"
    ...
    def test_security_hardening():
        """Test security hardening of the service"""
        cache.succeed("ps aux | grep gunicorn | grep pypi-cache")

        security_info = cache.succeed("systemctl show pypi-cache.service | grep -E '(ProtectSystem|PrivateTmp|NoNewPrivileges)'")
        assert "ProtectSystem=strict" in security_info, "ProtectSystem not strict"
        assert "PrivateTmp=yes" in security_info, "PrivateTmp not enabled"
        assert "NoNewPrivileges=yes" in security_info, "NoNewPrivileges not enabled"
    ...
    with subtest("Testing pip configuration"):
        test_pip_configuration()
